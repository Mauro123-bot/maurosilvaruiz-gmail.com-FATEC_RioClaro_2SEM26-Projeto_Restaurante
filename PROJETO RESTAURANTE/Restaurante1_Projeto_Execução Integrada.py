...
#########################################################################################################
# SCRIPT DE EXECUÇÃO E TESTE COMPLETO DO PROJETO INTEGRADO
#########################################################################################################

if __name__ == "__main__":
    print("--------------------------------------------------")
    print("  SISTEMA DE RESTAURANTE FATEC - EXECUÇÃO COMPLETA")
    print("--------------------------------------------------")

    # 1. Instanciação do Estoque e Produtos (Bloco 1)
    estoque = Estoque()
    ...
    estoque.adicionar_lote(LoteEstoque(hamburguer, quantidade=4, preco_compra=10.00, data_compra=date(2026, 9, 1), data_vencimento=date(2026, 10, 1)))
    estoque.adicionar_lote(LoteEstoque(suco, quantidade=10, preco_compra=3.00, data_compra=date(2026, 9, 2), data_vencimento=date(2026, 10, 5)))

    # 2. Instanciação da Cozinha e Caixa (Blocos 2 e 3)
    cozinha = FilaCozinha()
    caixa = Caixa()

    # 3. Execução das Funcionalidades do Bloco 4
    # Teste de Reposição
    gerenciador_reposicao = ReposicaoEstoque(estoque, limite_minimo=5)
    gerenciador_reposicao.verificar_e_repor(hamburguer, quantidade_compra=15, preco_compra=10.50)

    # Simulação de Vendas
    produtos_cadastrados = [hamburguer, suco]
    simular_atendimento_faker(estoque, cozinha, caixa, produtos_cadastrados)

    # Relatório Financeiro
    Relatorios.gerar_relatorio_financeiro(caixa)

    # Persistência de Dados (Pickle)
    estado_restaurante = {"estoque": estoque, "caixa": caixa}
    Persistencia.salvar_sistema(estado_restaurante)
    
    # Validação de Leitura
    dados_recuperados = Persistencia.carregar_sistema()