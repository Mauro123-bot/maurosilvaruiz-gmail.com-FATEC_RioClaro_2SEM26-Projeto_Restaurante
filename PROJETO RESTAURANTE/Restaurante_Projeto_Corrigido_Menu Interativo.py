...
if __name__ == "__main__":
    ...
    # MENU INTERATIVO
    # ------------------------------------------------------------------
    # IMPORTANTE: este menu usa input(), portanto precisa ser executado
    # em um TERMINAL/CONSOLE que aceite entrada pelo teclado.
    while True:
        print("\n" + "-" * 48, flush=True)
        print("       SISTEMA DE RESTAURANTE - PAINEL", flush=True)
        print("-" * 48, flush=True)
        print("1 - Criar/Abrir Comanda", flush=True)
        print("2 - Adicionar Item na Comanda", flush=True)
        print("3 - Processar Fila da Cozinha", flush=True)
        print("4 - Fechar Comanda e Receber Pagamento", flush=True)
        print("5 - Ver Relatório Financeiro", flush=True)
        print("6 - Rodar Simulação Automática (Aleatória)", flush=True)
        ...
        if opcao == "1":
            try:
                num_comanda = int(input("Número da Comanda: ").strip())
                num_mesa = int(input("Número da Mesa: ").strip())
            except ValueError:
                print("[ERRO] Digite números inteiros para comanda e mesa.")
                continue
            ...
            escolha = input("Escolha o produto (1 ou 2): ").strip()

            if escolha not in ("1", "2"):
                print("[ERRO] Produto inválido. Escolha 1 ou 2.")
                continue
            ...
            # Só altera a comanda se a baixa no estoque for realizada.
            if estoque.dar_baixa_fifo(prod.id_produto, qtd):
                obs = input("Observação (ex: Sem picles): ").strip() if escolha == "1" else ""
                if comanda_atual.adicionar_item(prod, qtd, obs):
                    if isinstance(prod, Comida):
                        cozinha.enfileirar_pedido(
                            comanda_atual,
                            comanda_atual.itens[-1]
                        )
            ...
            if comanda_atual is None or not comanda_atual.itens:
                print("[ERRO] Não há comanda aberta ou a comanda está sem itens.")
                continue

            print(
                f"\nTotal da Comanda #{comanda_atual.id_comanda}: "
                f"R${comanda_atual.calcular_total():.2f}"
            )
            ...
            # Aceita tanto "Cartão" quanto "Cartao", evitando erro por acento.
            formas = {
                "pix": "Pix",
                "cartao": "Cartão",
                "cartão": "Cartão",
            ...
            forma_normalizada = formas.get(forma.lower())

            if forma_normalizada is None:
                print("[ERRO] Forma de pagamento inválida.")
                print("Use: Pix, Cartão, Dinheiro ou Confiança.")
                continue
            ...
            if forma_normalizada == "Dinheiro":
                try:
                    valor_texto = input("Valor pago pelo cliente: R$ ").strip()
                    valor_texto = valor_texto.replace(",", ".")
                    val_entregue = float(valor_texto)
                except ValueError:
                    print("[ERRO] Digite um valor monetário válido.")
                    continue

            pagamento = caixa.processar_pagamento(
                comanda_atual,
                forma_normalizada,
                val_entregue
            )
            ...
            print("[ERRO] Opção inválida. Digite somente 0, 1, 2, 3, 4, 5 ou 6.")
